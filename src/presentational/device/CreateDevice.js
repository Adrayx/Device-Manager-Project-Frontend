...
export default function CreateUser() {
    ...
    return (
        <React.Fragment>
            <Paper sx={{ display: 'flex', flexDirection: 'column'}}>
                ...
                <Paper align={"center"} sx={{display: 'flex', flexDirection: 'row'}}>
                    ...
                    <Button
                        ...
                        startIcon={<Add/>}
                        onClick={() => {
                            if(device.description.length === 0 || device.address.length === 0 || device.threshold === -1)
                                handleClick();
                            else {
                                console.log(device);
                                client.insertDevice(device, localStorage.getItem("token"))
                                    .then(r => {
                                        if (r === null)
            ...
            >
                <DialogTitle>{"Devices list has been updated"}</DialogTitle>
                <DialogActions>
                    <Button href={`/devices`}>Ok</Button>
                </DialogActions>
            </Dialog>
            <Dialog
